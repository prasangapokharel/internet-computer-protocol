...
#[ic_cdk_macros::query(name = "hello")]
#[candid::candid_method(query, rename = "hello")]
async fn _cdk_user_defined_hello(_cdk_user_defined_name: String) -> (String) {
    let interpreter = unsafe { INTERPRETER_OPTION.as_mut() }
        .unwrap_or_trap("SystemError: missing python interpreter");
    let vm = &interpreter.vm;
    let params = (_cdk_user_defined_name
        .try_into_vm_value(vm)
        .unwrap_or_trap(),);
    call_global_python_function("hello", params)
        .await
        .unwrap_or_trap()
}
#[derive(
